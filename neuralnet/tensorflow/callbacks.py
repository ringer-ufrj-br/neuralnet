...
class SP(Callback):
    def __init__(
        self, validation_data, verbose=False, save_the_best=False, patience=False
    ):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        logs["val_max_sp_fa_val"] = fa[knee]
        logs["val_max_sp_pd_val"] = pd[knee]
        logs["val_max_sp_threshold"] = thresholds[knee]

        if self.verbose:
            logger.info(
                "val_sp: {:.4f} (fa:{:.4f},pd:{:.4f}), patience: {}".format(
                    sp[knee], fa[knee], pd[knee], self.count
                )
            )
